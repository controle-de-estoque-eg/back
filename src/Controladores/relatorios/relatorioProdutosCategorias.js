  ...
  try {
    const produtosMaisVendidos = await knex('vendas_produtos')
      .select(
        ...
        knex.raw(
          'sum(vendas_produtos.quantidade_produto) as quantidade_vendas'
        ),
        knex.raw('sum(vendas_produtos.valor_venda) as total_vendas')
      )
    ...
    const rentabilidadeProdutos = await knex('produtos')
      .select(
        'produtos.nome as produto',
        knex.raw(
          'sum(historico_venda.valor_venda - historico_custo.valor_custo) as lucro_total'
        )
      )
