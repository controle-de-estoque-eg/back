...
const relatorioVendas = async (req, res) => {
  ...
  try {
    const quantidadeVendas = await getQuantidadeVendas(
      periodoInicio,
      periodoFim
    )
    const ticketMedio = await getTicketMedio(periodoInicio, periodoFim)
    const quantidadeItensVendidos = await getQuantidadeItensVendidos(
      periodoInicio,
      periodoFim
    )
    const valorTotalVendas = await getValorTotalVendas(
      periodoInicio,
      periodoFim
    )
