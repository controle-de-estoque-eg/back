...
const relatorioLucro = async (req, res) => {
  ...
  try {
    ...
    const vendaLiquida = await knex('vendas')
      ...
      .select(
        knex.raw(
          'SUM(vendas.valor_venda - vendas.valor_custo) as venda_liquida'
        )
      )
      .first()
    ...
    const margemLucroCategoria = await knex('vendas_produtos')
      ...
      .select(
        ...
        knex.raw(
          'ROUND(((SUM(vendas_produtos.valor_venda) - SUM(vendas_produtos.valor_custo)) / SUM(vendas_produtos.valor_venda) * 100), 2) as margem_lucro'
        )
      )
