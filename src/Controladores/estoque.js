    ...
    if (fornecedor_id) {
      ...
      if (!fornecedor) {
        return res
          ...
          .json({
            mensagem:
              'Necessário um fornecedor válido para cadastrar um pedido',
          })
      }
    }
    const [cadastroEstoque] = await knex('movimento_estoque')
      .insert(dados)
      .returning('*')

    const { id: movimento_estoque_id } = cadastroEstoque

    const pedidoProduto = await cadastrarEstoqueProduto(
...
const excluirEstoque = async (req, res) => {
  ...
  try {
    ...
    await knex.transaction(async (trx) => {
      const movimentacao = await knex('movimento_estoque')
        .where({ id, soft_delete: false })
        .first()
      ...
      if (!movimentacao) {
        return res
          ...
          .json({
            mensagem:
              'A movimentação de estoque foi não encontrada ou já excluída.',
          })
      }
      ...
      await knex('produto_movimento_estoque')
        .where({ movimento_estoque_id: id, soft_delete: false })
        .update({ soft_delete: true })

      await knex('movimento_estoque')
        .where({ id, soft_delete: false })
        .update({ soft_delete: true })
    })
