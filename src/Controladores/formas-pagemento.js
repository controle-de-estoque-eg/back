...
const listarformas_pagemento = async (req, res) => {
    try {
        const formas_pagamento = await knex('formas_pagamento').where({ soft_delete: false })
        return (
...
const listarforma_pagemento = async (req, res) => {
    const { id } = req.params
    try {
        const formas_pagamento = await knex("formas_pagamento").where({ id, soft_delete: false }).first()
        if (!formas_pagamento) {
            return res.status(409).json({
                mensagem:
                    'A forma de pagamento informado não existe.',
            });
        }
        return (
            res.status(200).json(formas_pagamento)
...
const cadastrarformas_pagemento = async (req, res) => {
    const { nome } = req.body;
    try {
        const formas_pagamento = await knex('formas_pagamento').where({ nome, soft_delete: false }).first();
...
const editararformas_pagemento = async (req, res) => {
    ...
    const { id } = req.params
    try {
        const formas_pagamentoExiste = await knex('formas_pagamento').where({ id, soft_delete: false }).first();

        if (!formas_pagamentoExiste) {
            return res.status(404).json({ mensagem: 'A forma de pagamento informado não existe.' });
        }
        const formas_pagamento = await knex('formas_pagamento').where({ nome, soft_delete: false }).first();
        ...
        const formas_pagamentoAtualizadas = await knex('formas_pagamento')
            .where({ id })
            .update({ nome, update_at: DateTime.now().setZone('America/Sao_Paulo').toISO() })
            .returning("*");
...
const excluirformas_pagemento = async (req, res) => {
    ...
    try {
        ...
        await knex('formas_pagamento').where({ id }).update({
            soft_delete: true,
...
module.exports = { listarformas_pagemento, listarforma_pagemento, cadastrarformas_pagemento, editararformas_pagemento, excluirformas_pagemento }