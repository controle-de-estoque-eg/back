...
const cadastroProduto = async (req, res) => {
  ...
  const { nome, categoria_id, valor_custo, valor_venda, codigo_de_barras } =
    dados
  try {
    const nomeavalida = await knex('produtos')
      .where({ nome, soft_delete: false })
      .first()

    if (nomeavalida) {
      return res.status(409).json({
        mensagem: 'Ja existe um produto com esse nome',
      })
    }
    const codigoValido = await knex('produtos')
      .where({ codigo_de_barras, soft_delete: false })
      .first()

    if (codigoValido) {
      return res.status(409).json({
        mensagem: 'Ja existe um produto com esse codigo de barras',
      })
    }

    const categoriavalida = await knex('categorias')
      .where({ id: categoria_id, soft_delete: false })
      .first()
    ...
    if (valor_custo) {
      await knex('historico_custo').insert({
    ...
    if (valor_venda) {
      await knex('historico_venda').insert({
        produto_id: novoproduto[0].id,
        valor_venda,
      })
    }
...
const editarProduto = async (req, res) => {
  ...
  try {
    ...
    if (valor_custo) {
      await knex('historico_custo').insert({
        produto_id: produtoAtualizado[0].id,
        valor_custo,
      })
    }
...
const listarProduto = async (req, res) => {
  const { id } = req.params
  try {
    const detalharProduto = await knex('produtos')
      .where({ id, soft_delete: false })
      .first()
