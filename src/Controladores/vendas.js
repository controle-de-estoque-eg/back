...
const listarVenda = async (req, res) => {
  const { id } = req.params
  try {
    const pedidos = await knex('vendas')
      .where({ id, soft_delete: false })
      .first()
    if (!pedidos) {
      return res.status(409).json({
        mensagem: 'A venda informado não existe.',
      })
    }
    const lista_produtos = await knex('vendas_produtos').where({
      venda_id: id,
      soft_delete: false,
    })
...
const cadastrarVenda = async (req, res) => {
  ...
  try {
    ...
    if (cliente_id) {
      const clienteValido = await knex('clientes')
        .where({ id: cliente_id, soft_delete: false })
        .first()
      ...
      if (!clienteValido) {
        return res
          .status(400)
          .json({
            mensagem: 'Necessário um cliente válido para cadastrar um pedido',
          })
      }
    }
    ...
    const [cadastroVenda] = await knex('vendas')
    ...
    const cadastroPagamento = await formasDePagamento(
      venda_id,
      lista_pagamentos
    )
  ...
  try {
    ...
    await knex.transaction(async (trx) => {
      ...
      if (!venda) {
        return res
          .status(400)
          .json({ mensagem: 'Venda não encontrada ou já excluída.' })
      }

      // Atualizar formas de pagamento
      await knex('vendas_formas_pagamento')
        .where({ venda_id: id, soft_delete: false })
        .update({
          soft_delete: true,
          delete_at: DateTime.now().setZone('America/Sao_Paulo').toISO(),
        })
      ...
      const lista_produtos = await knex('vendas_produtos')
        .where({ venda_id: id, soft_delete: false })
        .transacting(trx)
